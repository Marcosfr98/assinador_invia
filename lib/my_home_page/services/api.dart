...
class MyHomePageApiServices extends ChangeNotifier {
  ...
  final _localPersistence = LocalPersistence.instance;
  ...
  Future<List<FluxoAguardandoModel>> getFluxosAguardando() async {
    try {
      await _localPersistence.getData(key: "aguardando", type: TypeVariable.int);
      final endpoint = Uri.parse('https://invianf.com.br/ws/appAssinador/aguardandoAssinatura.php');
      ...
      if (response.statusCode == 200) {
        ...
        List<FluxoAguardandoModel> fluxos = await jsonObject
            .map(
  ...
  Future<List<FluxosPendentesModel>> getFluxosPendentes() async {
    try {
      ...
      final endpoint = Uri.parse('https://invianf.com.br/ws/appAssinador/fluxosPendentes.php');
      final payload = jsonEncode(
      ...
      if (response.statusCode == 200) {
        ...
        await _localPersistence.setData(
          key: "pendentes",
          type: TypeVariable.int,
          value: fluxosPendentesModel.length,
        );
  ...
  Future<List<FluxosFinalizadosModel>> getFluxosFinalizados() async {
    try {
      await _localPersistence.getData(key: "pendentes", type: TypeVariable.int);
      final endpoint = Uri.parse('https://invianf.com.br/ws/appAssinador/fluxosFinalizados.php');
      final payload = jsonEncode(
      ...
      if (response.statusCode == 200) {
        ...
        List<FluxosFinalizadosModel> fluxos = jsonObject
            .map(
  ...
  Future<List<AssinanteFluxoModel>> getAssinantes({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<AssinanteFluxoModel> assinantesFluxoModel = json.map((assinante) => AssinanteFluxoModel.fromJson(assinante)).toList().cast<AssinanteFluxoModel>();
  ...
  Future<void> sendEmailToSigners({
    ...
    for (var assinante in assinantes) {
      if (assinante.assinado == "1") {
        ...
        final headers = {'api-key': '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'};
        final endpoint = Uri.parse("https://api.sendinblue.com/v3/smtp/email");
        final payload = jsonEncode({
          "sender": {"name": "Equipe INVIA ASSINADOR", "email": "webpki@webpki.com.br"},
          "to": [
            {
        ...
        http.Response response = await http.post(endpoint, body: payload, headers: headers);
        if (response.statusCode == 200) {
          print(response.body);
        } else {
          print(response.body);
