    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Builder(
            builder: (context) {
              if (widget.item is FluxosPendentesModel || widget.item is FluxosFinalizadosModel) {
                return IconButton(
                  onPressed: () async {
                    Get.to(
                      () => PdfViewer(
                ...
                ElevatedButton(
                  onPressed: () async {
                    try {
                      ...
                      String linkAssinatura = "https://invianf.com.br/inviassinador/signatureDetails.php?code=$idHash";
                      for (var assinante in assinantesFluxoModel) {
                        if (assinante.assinado == "1") {
                          print("Já assinou");
                        } else {
                          final headers = {'api-key': '${_db.keys["Sendiblue"]}'};
